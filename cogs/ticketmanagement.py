...
#         await createticket(self.bot, interaction.message.guild, self.children[0].value, interaction.user)
...
#         print(type(lookup["needreason"]))
#         if lookup["needreason"] == True:
#             modal = Modal(self.bot, title="Create ticket")
#         else:
#             modal = NotRequiredModal(self.bot, title="Create ticket")
#         await interaction.response.send_modal(modal)

# async def createticket(bot, guild, reason, user):
...
#             lookup = await bot.db.fetchrow("SELECT * FROM tickets WHERE userid = $1 AND guild = $2", user.id, guild.id)
#             if lookup == None:
#                 embed = discord.Embed(title="Ticket", description=f"You do not have a ticket open.", color=0x00b2ff)
#                 await interaction.response.send_message(embed=embed, ephemeral=True)
#                 return
#             await bot.db.execute("DELETE FROM tickets WHERE userid = $1 AND guild = $2", user.id, guild.id)
#             await self.channel.delete()
#             self.stop()

#     guildconfig = await bot.db.fetchrow("SELECT * FROM ticketconfigs WHERE guild = $1", guild.id)
#     if guildconfig is None:
#         return
    
#     try:
#         category = bot.get_channel(guildconfig["category"])
#     except:
#         return

#     embed = discord.Embed(title="Ticket", description=f"{user.mention}, has made a ticket with {reason}.\n\nPlease wait for a staff member to respond. (Will automatically delete in 30 minutes)", color=0x00b2ff)
#     embed.set_footer(text=f"Ticket ID: {user.id}")

#     channel = await guild.create_text_channel(f"ticket-{user.name}", category=category, topic=reason, reason="Creating ticket")
#     view = CloseTicket(channel)
